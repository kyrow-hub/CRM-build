...
| Dashboard | `clients`, `referrals`, `meetings`, `client_notes`, `case_activities`, `client_outcomes`, `good_news_stories` | Active Clients / Referrals This Week / Meetings This Week / Reviews Overdue / Compliance Alerts / Open Incidents stat cards, plus a real recent-activity feed aggregated across several tables. |
...
| Clients (list) | `clients` | Full CRUD, search, status filtering, archive, and a permanent "Delete Permanently" hard-delete on the client's own profile header (admin/manager only, separate from Archive - cascades to the client's case notes, activities, goals, outcomes, documents, staff register, follow-ups, assessments, service plan items, and Family & Contacts entries; unlinks rather than deletes their referrals, meetings, good news stories, incidents, SMS, and emails). Each client has a `client_type` (Case Managed / Activity Only, set at creation and editable) - Activity Only clients are marked with a badge and are held to a reduced compliance checklist (see Compliance tab below). |
...
| Family & Contacts | `client_relationships` | Guardians, parents, siblings, and other emergency contacts, each with relationship type, phone, email, address, and a primary-contact flag. Edit for any editor role, Delete restricted to admins/managers. |
